...
async def process_chat_llama3(message: str) -> str:
    ...
    try:
        ...
        if (parsed["result"] == '0'):
            ...
            reflect_chain = reflect_prompt | llm | StrOutputParser()
            final_answer = await reflect_chain.ainvoke({ "question": message}, config={"callbacks":[langfuse_handler]})
            print(f"[3️⃣ Reflect 결과] {final_answer}")
            return final_answer
...
async def process_chat_qwen(message: str) -> str:
    """사용자 입력 → LLM → MCP 호출 → 응답 생성"""
    print("---------process_chat start---------")
    ...
    # ✅ Qwen2.5 1.5B Instruct (양자화 Q4_K_M) 모델로 변경
    llm = ChatOllama(
    ...
    try:
        ...
        print(f"[LLM 응답 cleaned] {cleaned}")
        if parsed.get("result") == "0":
            ...
            if response.status_code == 200:
                ...
                reflect_prompt = PromptTemplate.from_template(
                    ...
                    ### 지시 
                    너는 친절한 AI 어시스턴트야.
                    다음 사용자의 질문과 mcp 서버 조회 결과를 보고 답변을 만들어 줘
                    ...
                    ### 답변 생성
                    1. 결과의 result 값이 -1 이면 질문에 대한 자연스럽고 간결히 답변
                    2. result 값이 0 이면 mcp 서버 결과를 통해 사용자 질문에 대한 답변을 만들어 줘
                    3. 무조건 일반 text 형식으로 답변 부탁해
                    4. 사용자는 무조건 한국인이니 한국어로 답해
                    답변:
                    """
                )
