...
def build_summaries_chunk(content_chunks):
    ...
    for summary_path, info in summaries.items():
        ...
        summary_id_map[summary_path] = sid

    return summary_chunks, summary_id_map

# parent id 연결
def attach_parent_ids(content_chunks, summary_id_map):
    def split_path(section_title: str):
        return [p.strip() for p in section_title.split('>') if p.strip()]
    ...
    for ch in content_chunks:
        ...
        # 상위 경로 중 가장 깊은 summary 매칭
        parent_id = None

        for depth in reversed(range(1, len(parts))):
            summary_path = " > ".join(parts[:depth])
            if summary_path in summary_id_map:
                parent_id = summary_id_map[summary_path]
                break
